...
_RESTORE_USERS_CREATES=( $(echo "$USER_DATAPUMP_RESTORE_TO" | tr "," " ") )

if [[ "$_CREATE_USER" == "true" ]]; then  
  for _USER_CREATE in ${_RESTORE_USERS_CREATES[@]} 
  do        
...
PREFIX_KEY_POS_RESTORE="${PREFIX_KEY}_POS"
PREFIX_KEY_POS_RESTORE_EXEC="${PREFIX_KEY_POS_RESTORE}_EXEC"
_POS_RESTORE_EXEC="${!PREFIX_KEY_POS_RESTORE_EXEC:-$ORACLE_DATAPUMP_RESTORE_POS_EXEC}"

if [[ "$_POS_RESTORE_EXEC" == "true" ]]; then  
  for _USER_CREATE in ${_RESTORE_USERS_CREATES[@]} 
  do        
    PREFIX_KEY_POS_RESTORE_USER="${ORACLE_DATAPUMP_RESTORE_POS_PREFIX_ENVS}_${_USER_CREATE}" 
